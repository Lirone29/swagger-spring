...
@RestController
public class FollowingListApiController implements FollowingListApi {

    private static final Logger log = LoggerFactory.getLogger(FollowingListApiController.class);
    ...
    public FollowingListApiController(ObjectMapper objectMapper, HttpServletRequest request) {
    ...
    public ResponseEntity<FollowingList> getUsersFromFollowingList(@Parameter(in = ParameterIn.PATH, description = "ID of user to access all followed users", required=true, schema=@Schema()) @PathVariable("userId") Long userId) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<FollowingList>(objectMapper.readValue("{\n  \"id\" : 0,\n  \"userId\" : 6,\n  \"otherUserId\" : 1\n}", FollowingList.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<FollowingList>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<FollowingList>(HttpStatus.NOT_IMPLEMENTED);
    }
